...
ggplot(aes(x=disp,y=wt),data=mtcars) + geom_point()
ggplot(aes(x=disp,y=wt),data=mtcars) + geom_point()
ggplot(data=mtcars,aes(x=cyl)) + geom_bar(binwidth=1)
qplot(data=mtcars,x=cyl,binwidth=1)
...
statesinfo$state.abb

ggplot(aes(x=income,y=life.exp),data=statesinfo) + geom_point() + geom_smooth()
cor(statesinfo$income,statesinfo$life.exp)
#Adding labels to the points in the scatterplot
ggplot(aes(x=income,y=life.exp,color=income),data=statesinfo) + 
  geom_point() + 
...
#Subsetting the data based on the region

first <- subset(statesinfo,state.region==1)
head(first,2)
tail(first,2)
dim(first)

second <- subset(statesinfo,state.region==2)
head(second,2)
tail(second,2)
dim(second)

third<-subset(statesinfo,state.region==3)
head(third)
dim(third)
tail(third,3)

tetra <- subset(statesinfo,state.region==4)
head(tetra,2)
tail(tetra,2)
dim(tetra)
min(tetra$illiteracy)
max(tetra$illiteracy)

ggplot(data=statesinfo,aes(x=murder,y=illiteracy,color=state.abb)) + 
    geom_point() + geom_text(aes(label=state.abb))


ggplot(data=statesinfo,aes(x=murder,y=illiteracy,color=income)) + 
  geom_point() + geom_text(aes(label=state.abb))
...
qplot(data=reddit,x=age.range,order=c)

#Ordering of the bars in the bar plot
reddit$age.range <- ordered(reddit$age.range,levels=c("Under 18", "18-24", "25-34", "35-44", "45-54", "55-64", "65 or Above"))
qplot(data=reddit,x=age.range)

#Removing the NA values from the age.range
qplot(data=subset(reddit,!is.na(reddit$age.range)),x=age.range,fill="maroon")

ggplot(data=subset(reddit,!is.na(reddit$age.range)),aes(x=age.range)) + 
  geom_histogram(fill="blue") + scale_y_continuous(labels=comma) 

ggplot(data=subset(reddit,!is.na(reddit$age.range)),aes(x=age.range)) + 
  labs(title="Distribution of the Age Range among the reddit users",x="Age Ranges",y="Count of records in the dataset for each age range") +
  geom_histogram(fill="blue") + 
  scale_y_continuous(labels=comma) 

ggsave("reddit_agerange.jpg",width = 10,height = 5,dpi = 300)
